...
def _ensemble_rate_forecast(**kwargs):
    ...
    ds['precip'].attrs.update(units='mm day-1', standard_name='lwe_precipitation_rate', long_name='Precipitation rate')
    return ds
...
def test_replot_from_spec(tmp_path, plot_mediogram):
    ...
    run_skill(plot_mediogram, '-i', str(fc), '-i', str(mc), '--dump-spec', str(tmp_path / 'medio.plot.json'), '--spec', '{"geo":{"lat":1.0,"lon":10.0},"title":"Original"}')
...
def test_patch_flag_merges_into_spec(tmp_path, plot_mediogram):
    ...
    mc = write_zarr(_ensemble_rate_forecast(members=5, n_step=4, fill=0.5), tmp_path / 'mc.zarr')
    out = tmp_path / 'medio.png'
    run_skill(plot_mediogram, '-i', str(fc), '-i', str(mc), '--dump-spec', str(tmp_path / 'medio.plot.json'), '--spec', '{"geo":{"lat":1.0,"lon":10.0},"title":"Patched"}')
