...
def test_metric_bias_writes_png(tmp_path, plot_fn, verify_fn, capsys):
    ...
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '-o', str(out))
    assert Path(out).exists()
...
def test_replot_from_spec(tmp_path, plot_fn, verify_fn):
    ...
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '--dump-spec', str(tmp_path / 'verify_bias.plot.json'), '--spec', '{"title":"Original"}')
...
def test_patch_flag_merges_into_spec(tmp_path, plot_fn, verify_fn):
    ...
    _run_verify(verify_fn, fc, obs, vpath, metric='bias')
    out = tmp_path / 'verify_bias.png'
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '--dump-spec', str(tmp_path / 'verify_bias.plot.json'), '--spec', '{"title":"Patched"}')
...
def test_row_labels_explicit_overrides(plot_mod):
    ...
    a = _week(event_at=[])
    b = _week(event_at=[])
    assert plot_mod._row_labels(obs, [a, b], 'hits', labels=['Obs custom', 'Fc A', 'Fc B']) == ('Obs custom', 'Fc A / Fc B', 'Hits')
...
def test_custom_lead_labels(tmp_path, plot_fn, verify_fn, capsys):
    ...
    out = tmp_path / 'verify.png'
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(a), '--verify', str(va), '--forecast', str(b), '--verify', str(vb), '-o', str(out), '--spec', '{"traces":[{"leads":["W1","W2"]}]}')
...
def test_lead_count_mismatch_is_refused(tmp_path, plot_fn, verify_fn):
    ...
    with pytest.raises(SystemExit) as exc:
        run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '-o', str(tmp_path / 'out.png'), '--spec', '{"traces":[{"leads":["W1","W2"]}]}')
    assert exc.value.code == 2
...
def test_obs_finer_grid_is_refused(tmp_path, plot_fn, verify_fn):
    fc = write_zarr(make_gridded(n_time=1, fill=5.0, lats=(1.0, 2.0), lons=(10.0, 11.0)), tmp_path / 'fc.zarr')
    obs = write_zarr(make_gridded(n_time=1, fill=5.0, lats=(1.0, 1.5, 2.0), lons=(10.0, 10.5, 11.0)), tmp_path / 'obs.zarr')
    vpath = tmp_path / 'v.zarr'
    with pytest.raises(SystemExit):
        _run_verify(verify_fn, fc, obs, vpath)
...
def test_multi_time_obs_is_refused(tmp_path, plot_fn, verify_fn):
    ...
    _run_verify(verify_fn, fc, obs, vpath)
    with pytest.raises(SystemExit) as exc:
        run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '-o', str(tmp_path / 'out.png'))
    assert exc.value.code == 2
...
def test_bbox_slices_before_draw(tmp_path, plot_fn, verify_fn):
    ...
    _run_verify(verify_fn, fc, obs, vpath)
    out = tmp_path / 'verify.png'
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(fc), '--verify', str(vpath), '-o', str(out), '--spec', '{"geo":{"bbox":[3.0,9.0,0.0,12.0]}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0

def test_order_week1_first_sorts_week_labels(plot_mod):
    leads, forecasts, verifies, labels = plot_mod._order_week1_first(['Week 4 (init Sep 1)', 'Week 1 (init Sep 22)'], ['w4', 'w1'], ['v4', 'v1'], ['Obs', 'GEFS w4', 'GEFS w1'])
...
def test_order_week1_first_keeps_custom_labels(plot_mod):
    leads, forecasts, verifies, labels = plot_mod._order_week1_first(['W1', 'W2'], ['a', 'b'], ['va', 'vb'], None)
    assert leads == ['W1', 'W2']
...
def test_week_labels_print_week1_to_week4(tmp_path, plot_fn, verify_fn, capsys):
    ...
    out = tmp_path / 'verify.png'
    run_skill(plot_fn, '--obs', str(obs), '--forecast', str(w4), '--verify', str(v4), '--forecast', str(w1), '--verify', str(v1), '-o', str(out), '--spec', '{"traces":[{"leads":["Week 4","Week 1"]}]}')
    printed = capsys.readouterr().out
