...
from weather_skills_core.plot import charts as plot_charts
from weather_skills_core.plot import figure as ws_figure
from weather_skills_core.plot import maps as plot_maps
from weather_skills_core.plot import spec as ws_spec
from weather_skills_core.plot import theme as plot_theme
...
def test_fontsize_writes_png(tmp_path, plot_fn):
    src = write_zarr(make_gridded(), tmp_path / 'in.zarr')
    out = tmp_path / 'map.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"theme":{"fontsize":22},"title":"Large"}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_timeseries_legend_writes_png(tmp_path, plot_fn):
    src = write_zarr(make_gridded(), tmp_path / 'in.zarr')
    out = tmp_path / 'ts.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"timeseries","reduce":["latitude","longitude"]}],"legend":"upper right","layout":{"figsize":[9.0,4.0]}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_contour_stamps_history(tmp_path, plot_fn):
    src = write_zarr(make_gridded(), tmp_path / 'in.zarr')
    out = tmp_path / 'contour.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"contour"}],"title":"Isolines"}')
    history = load_figure_history(out)
...
def _map_figure(ds, *, fontsize=16, **spec_extra):
    ...
    import matplotlib
    matplotlib.use('Agg')
    spec = {'version': 2, 'inputs': [{'id': 'a', 'variable': next(iter(ds.data_vars))}], 'traces': [{'kind': 'heatmap', 'input': 'a'}], 'layout': {'facet': {}}, 'theme': {'colormap': 'viridis'}, 'geo': {}}
...
def test_long_title_still_renders():
    import matplotlib.pyplot as plt
    fig = _map_figure(make_gridded(n_time=1), extent=[10.0, 11.0, 1.0, 2.0], title='Kenya GEFS vs CHIRPS 5 mm event verification · 2026-08-04 to 2026-08-10')
    fig.canvas.draw()
...
def test_heatmap_axis_label_overrides(tmp_path, plot_fn):
    fig = _map_figure(make_gridded(n_time=1), fontsize=14, extent=[10.0, 11.0, 1.0, 2.0], xlabel='Eastings', ylabel='Northings')
...
def test_subplot_title_overrides_heatmap_panels():
    fig = _map_figure(make_gridded(n_time=2), fontsize=14, extent=[10.0, 11.0, 1.0, 2.0], title='Season', subplot_titles=['Week 1', 'Week 2'], cbar_label='Rain (mm)')
...
def test_theme_rc_axes_titlesize_sets_panel_title_size():
    fig = _map_figure(make_gridded(n_time=2), fontsize=16, extent=[10.0, 11.0, 1.0, 2.0], theme={'colormap': 'viridis', 'rc': {'axes.titlesize': 10}})
...
def test_panel_title_calendar_weekly_range(plot_fn):
    ...
    da = xr.DataArray(np.zeros((len(times), 2, 2)), dims=('time', 'latitude', 'longitude'), coords={'time': times, 'latitude': [0.0, 1.0], 'longitude': [36.0, 37.0]}, name='precip')
...
def test_timeseries_forecast_writes_png(tmp_path, plot_fn):
    ...
    src = write_zarr(ds, tmp_path / 'in.zarr')
    out = tmp_path / 'ts.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"timeseries","reduce":["latitude","longitude"]}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_timeseries_along_draws_one_line_per_member(tmp_path, plot_fn):
    ...
    src = write_zarr(ds, tmp_path / 'in.zarr')
    out = tmp_path / 'ts.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"timeseries","reduce":["latitude","longitude"],"along":"number"}]}')
    assert Path(out).exists() and out.stat().st_size > 0

def test_precip_default_colormap_is_nested_window():
    from matplotlib.colors import BoundaryNorm, ListedColormap
    from weather_skills_core.plot.theme import precip_nested_palette
    da = make_forecast()['tp']
    da.attrs.update(units='mm', standard_name='lwe_thickness_of_precipitation_amount', aggregation_period='10 day')
...
def test_precip_short_period_colormap_uses_daily_window():
    ...
    da.attrs.update(units='mm', standard_name='lwe_thickness_of_precipitation_amount', aggregation_period='1 day')
    cmap, norm = plot_maps._heatmap_scale(da, None)
...
def test_precip_anomaly_colormap_is_nested_week_window():
    ...
    named.attrs.update(units='mm', standard_name='lwe_thickness_of_precipitation_amount', long_name='rainfall anomaly')
...
def test_layer_vmin_vmax_option(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{src}', '-o', str(out), '--spec', '{"layers":[{"id":"a","vmin":0.0,"vmax":15.0}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_layer_inherits_figure_colormap_and_vlim(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{src}', '-o', str(out), '--spec', '{"theme":{"colormap":"RdBu_r"},"layers":[{"id":"a","variable":"sst","vmin":-1.5,"vmax":1.5}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0

def test_amount_colorbar_drops_leftover_rate_name():
    da = make_forecast()['tp']
    da.attrs.update(units='mm', standard_name='lwe_thickness_of_precipitation_amount', long_name='precipitation rate', GRIB_name='Precipitation rate')
...
def test_heatmap_draw_box_writes_png(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"geo":{"draw_boxes":[[10.0,50.0,-10.0,70.0],[0.0,90.0,-10.0,110.0]]}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_flag_field_heatmap_writes_png(tmp_path, plot_fn):
    ...
    ds['event_hit'].attrs.update(units='1', long_name='Event verification', flag_values=np.array([-1, 0, 1], dtype=np.int8), flag_meanings='disagree below hit')
...
def test_windrose_legend_below_writes_png(tmp_path, plot_fn):
    src = write_zarr(_make_wind(), tmp_path / 'wind.zarr')
    out = tmp_path / 'rose.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"windrose"}],"legend":"below","layout":{"figsize":[6.0,6.0]}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_windrose_stamps_history(tmp_path, plot_fn):
    src = write_zarr(_make_wind(), tmp_path / 'wind.zarr')
    out = tmp_path / 'rose.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"windrose"}],"title":"10 m wind"}')
    history = load_figure_history(out)
    assert history is not None
    assert history[-1]['skill'] == 'plot'
...
def test_windrose_forecast_and_explicit_vars(tmp_path, plot_fn):
    ...
    out = tmp_path / 'rose.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"windrose","u_variable":"u10","v_variable":"v10"}],"inputs":[{"index":"step=0"}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_windrose_bbox_writes_png(tmp_path, plot_fn):
    ...
    out = tmp_path / 'rose.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"windrose"}],"geo":{"bbox":[3.0,32.0,-3.0,42.0]}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_quiver_stamps_history(tmp_path, plot_fn):
    src = write_zarr(_make_wind(), tmp_path / 'wind.zarr')
    out = tmp_path / 'quiver.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"quiver"}],"title":"10 m wind"}')
    history = load_figure_history(out)
    assert history is not None
    assert history[-1]['skill'] == 'plot'
...
def test_quiver_forecast_panels(tmp_path, plot_fn):
    ...
    out = tmp_path / 'quiver.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"quiver","u_variable":"u10","v_variable":"v10","quiver":{"scale":40.0}}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def _write_box_geojson(path, lon_min=9.5, lon_max=13.5, lat_min=0.5, lat_max=3.5):
    import json
    path.write_text(json.dumps({'type': 'FeatureCollection', 'features': [{'type': 'Feature', 'properties': {}, 'geometry': {'type': 'Polygon', 'coordinates': [[[lon_min, lat_min], [lon_max, lat_min], [lon_max, lat_max], [lon_min, lat_max], [lon_min, lat_min]]]}}]}))
    return path
...
def test_heatmap_and_layer_heatmap_are_pixel_identical(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '-i', str(src), '-o', str(heatmap), '--spec', '{"traces":[{"kind":"heatmap"}]}')
...
def test_layer_heatmap_scatter_outline(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{grid}', '--layer', f'scatter:{pts}', '--layer', f'outline:{geo}', '-o', str(out))
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_xy_writes_png_pair_on_year(tmp_path, plot_fn):
    ...
    iod['iod_mode_index'].attrs.update(units='degree_Celsius', long_name='IOD', standard_name='sea_surface_temperature_anomaly')
    ...
    rain['precip'].attrs.update(units='mm', long_name='October rainfall', standard_name='lwe_thickness_of_precipitation_amount')
    x_src = write_zarr(iod, tmp_path / 'iod.zarr')
    y_src = write_zarr(rain, tmp_path / 'rain.zarr')
    out = tmp_path / 'xy.png'
    run_skill(plot_fn, '--x', str(x_src), '--y', str(y_src), '-o', str(out), '--spec', '{"traces":[{"kind":"xy","pair_on":"year"}]}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
    history = load_figure_history(out)
    assert history[-1]['skill'] == 'plot'
...
def test_xy_same_input_two_variables(tmp_path, plot_fn):
    ...
    ds['iod_mode_index'].attrs.update(units='degree_Celsius', long_name='IOD', standard_name='sea_surface_temperature_anomaly')
    src = write_zarr(ds, tmp_path / 'both.zarr')
    out = tmp_path / 'xy.png'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--spec', '{"traces":[{"kind":"xy","x_variable":"iod_mode_index","y_variable":"precip"}]}')
    assert Path(out).exists()

def test_xy_requires_both_x_and_y(tmp_path, plot_fn):
    src = write_zarr(make_gridded(), tmp_path / 'in.zarr')
    with pytest.raises(SystemExit):
        run_skill(plot_fn, '--x', str(src), '-o', str(tmp_path / 'out.png'), '--spec', '{"traces":[{"kind":"xy"}]}')

def test_xy_rejects_layer(tmp_path, plot_fn):
    src = write_zarr(make_gridded(), tmp_path / 'in.zarr')
    with pytest.raises(SystemExit):
        run_skill(plot_fn, '--layer', f'heatmap:{src}', '-o', str(tmp_path / 'out.png'), '--spec', '{"traces":[{"kind":"xy"}]}')
...
def test_layer_independent_scale(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{grid}', '--layer', f'heatmap:{other}', '-o', str(out), '--spec', '{"layers":[{"id":"a","variable":"precip"},{"id":"b","variable":"t2m"}],"layout":{"shared_colorscale":false}}')
    assert Path(out).exists()
    assert out.stat().st_size > 0
...
def test_heatmap_dump_spec_on_request(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"title":"Precip"}')
...
def test_heatmap_panel_spacing_dump_spec(tmp_path, plot_fn):
    ...
    spec_path = tmp_path / 'map.plot.json'
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"layout":{"facet":{"wspace":0.4,"hspace":0.2}}}')
    spec = json.loads(spec_path.read_text())
...
def test_dump_spec_skips_png_even_when_output_is_set(tmp_path, plot_fn):
    ...
    out = tmp_path / 'map.png'
    spec_path = tmp_path / 'map.plot.json'
    run_skill(plot_fn, '-i', str(src), '-o', str(out), '--dump-spec', str(spec_path), '--spec', '{"title":"Precip"}')
...
def test_replot_from_spec(tmp_path, plot_fn):
    ...
    spec_path = tmp_path / 'map.plot.json'
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"title":"Original"}')
...
def test_patch_flag_merges_into_spec_and_spec_patch_key_is_refused(tmp_path, plot_fn, capsys):
    ...
    out = tmp_path / 'map.png'
    spec_path = tmp_path / 'map.plot.json'
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"title":"Patched"}')
...
def test_windrose_dump_spec_on_request(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"traces":[{"kind":"windrose"}],"title":"Rose"}')
...
def test_quiver_dump_spec_on_request(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '-i', str(src), '--dump-spec', str(spec_path), '--spec', '{"traces":[{"kind":"quiver"}],"title":"Wind"}')
...
def test_xy_dump_spec_on_request(tmp_path, plot_fn):
    ...
    y_src = write_zarr(rain, tmp_path / 'rain.zarr')
    out = tmp_path / 'xy.png'
    run_skill(plot_fn, '--x', str(x_src), '--y', str(y_src), '--dump-spec', str(tmp_path / 'xy.plot.json'), '--spec', '{"traces":[{"kind":"xy","pair_on":"year"}],"title":"IOD vs rain"}')
...
def test_layer_dump_spec_on_request(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{src}', '--dump-spec', str(spec_path), '--spec', '{"title":"Layered"}')
...
def test_layer_patch_by_id_dump_spec(tmp_path, plot_fn):
    ...
    run_skill(plot_fn, '--layer', f'heatmap:{src}', '--layer', f'heatmap:{src}', '--dump-spec', str(spec_path), '--spec', '{"layers":[{"id":"a","variable":"sst","colormap":"RdBu_r","vmin":-1.5},{"id":"b","variable":"sst"}]}')
