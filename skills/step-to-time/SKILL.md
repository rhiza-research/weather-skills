---
name: step-to-time
description: Realize a forecast dataset's `step` lead-time axis as wall-clock valid times (`time = init + step`), replacing the `step` dim with a `time` dim. Use it to compare a forecast against observations — e.g. before plot-timeseries, difference, or verify against a time-based dataset.
license: MIT
...
- To compare a forecast against observations: run it on the forecast before
  feeding both inputs to `plot-timeseries`, `difference`, or `verify`
